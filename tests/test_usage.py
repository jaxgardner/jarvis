...
def test_say_records_tokens_on_the_utterance(client, db, monkeypatch):
    monkeypatch.setattr(
        router,
        "route",
        lambda text, tz, reports=(), projects=(): (usage.record(fake_usage(2557, 95)), ("add_note", {"body": text}))[1],
    )
...
def test_a_two_hop_query_bills_both_calls(client, db, monkeypatch):
    ...
    monkeypatch.setattr(
        router,
        "route",
        lambda text, tz, reports=(), projects=(): (
...
def test_metrics_reports_spend(client, db, monkeypatch):
    monkeypatch.setattr(
        router,
        "route",
        lambda text, tz, reports=(), projects=(): (usage.record(fake_usage(2000, 100)), ("add_note", {"body": text}))[1],
    )
